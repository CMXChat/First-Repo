...
Date: 2026-08-18
Status: Canonical product/UX direction for Directory; Lab Directory v2 implemented, protected backend expansion pending
...
The main `/lab/` Records area now has a richer Directory v2 product layer.
...
- `cmx-lab-crm-v1` remains the shared browser-local store so older Lab surfaces and focused Automations continue to see the same People and Organizations;
- `cmx-lab-directory-ui-v2` stores Directory-only view/navigation preferences.

Directory v2 is additive. It preserves older compatibility fields such as `orgId`, `email`, `phone` and `tags` while adding richer prototype fields such as:
...
Directory v2 currently exposes three primary object views:
...
The Lab still mirrors preferred email and phone into the older scalar fields for compatibility.
...
Relationship type is descriptive context. Separate policy decides whether a relationship grants any authority.
...
The Automation definition should store stable protected audience selector IDs instead of copied mutable recipient strings.
...
Current focused Automations v4 integration:

- direct Person targets work in the existing Lab editor;
- direct Organization targets work in the existing Lab editor;
- target picker rows now show Directory email/phone readiness and Organization member readiness;
- the Actions stage shows current Directory counts;
- Review shows Directory readiness counts;
- saved Groups are visible in the target picker as an audience preview;
- direct Group/Label selection is intentionally **not** faked through the older single-target field.

The next real audience-editor milestone is a typed multi-selector Audience model that can represent Person / Organization / Group / Label references without corrupting the existing target field.
...
Future provider delivery must preserve exact skipped/failure reasons rather than silently dropping unresolved recipients.

## Directory and Library

Directory relationships should eventually be able to project linked Library information without making folder placement a permission boundary.
...
Do not interrupt the already validated Phase 2A production migration merely because Directory v2 now exists in Lab.
...
8. protected search/list/detail APIs;
9. activity/note projection;
10. duplicate detection and explicit merge;
11. custom fields and saved views when needed;
12. import/export;
13. deeper communication/Runtime history integration.
