...
The primary navigation calls the full module index `Explore`. The opening action, recommendation card, discovery previews, and mobile section pager all lead into it. Inside Explore, a compact cue tells the person that the category names are controls. A named continuation action identifies what comes next, and the progress label shows the current position. Every section exposes the complete category index near its opening through five `Explore the full picture` cards. The current category is marked `Viewing now`, while every other card is an equally visible destination with the real section summary. Stronger depth, numbered background marks, restrained tonal variation, and orbital details make this index the visual center of Explore. The index appears as a complete row on desktop and becomes a swipeable rail on mobile. After each detailed module, two `Continue the thread` cards offer the next useful connected contexts. Every category-changing control resets the old deep scroll position and aligns the selected category's opening navigation below the rendered sticky topbar. The `Everything` view reuses the same specialized calendars, habit trackers, boards, ledgers, portfolios, and operational dashboards as Explore and keeps those sections free of generic reductions; it renders those heavier modules only while Everything is open. Its sticky horizontal section index highlights the current category, keeps that category in view, jumps directly to every major section, and fills a thin progress rail as the page moves toward completion. Everything jumps account for both sticky bars so the destination heading remains visible. The opening `Scroll through today` control moves to the next visible part of the Brief and respects reduced-motion preferences.
...
The end of Everything includes a distinct final bridge to `/doc/` after the focused-view continuation controls. Everything's Numbers, Flow, Spaces, Adaptive Brief, Morning concept, and Privacy sections use composed visuals rather than plain repeated cards. These include a signal console, connected live sequence, permission boundary map, adaptive view orchestrator, device and voice concept, layered privacy controls, and a recent control log. These visuals remain honest interface demonstrations and do not imply a working private-data backend.

The Team and Project briefing uses a command view before its detailed workstream list. It combines an overall health dial, open-work trajectory, named-owner coverage, blocker count, review window, and release state. This keeps project status legible as an operating picture while the detailed rows remain available for ownership and next evidence. Status boards, metric panels, progress views, and handoff sequences use brighter section-specific color fields so different kinds of information no longer collapse into identical white cards.
