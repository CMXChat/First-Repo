...
The primary navigation calls the full module index `Explore`. The opening action, recommendation card, discovery previews, and mobile section pager all lead into it. Inside Explore, a compact cue tells the person that the category names are controls. Persistent previous and next arrows open the adjacent section, a named continuation action identifies what comes next, and the progress label shows the current position. Every section exposes the complete category index near its opening through `All sections in this briefing` cards. The current category is marked `Viewing now`, while every other card is an equally visible destination with the real section summary. A restrained blue depth treatment, soft elevation, and partial orbital detail make this index the visual center of Explore without assigning unrelated colors to each category. The index appears as a complete row on desktop and becomes a swipeable rail on mobile. This keeps the full depth visible before someone reaches the current section's longer data view. The opening `Scroll through today` control moves to the next visible part of the Brief and respects reduced-motion preferences.
