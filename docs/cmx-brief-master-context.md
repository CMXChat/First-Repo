...
This file is the current product and technical context for the Personal OS demonstration, its product overview, and the future platform direction. It replaces the August 4 architecture assumptions that described the older modular Brief runtime as the active production implementation.

For the detailed code comparison, read `docs/2026-08-05-repository-reconciliation.md`.
...
The primary public noindex product demonstration.
...
- scenario-specific people, priorities, permissions, stats, Spaces, and actions
- responsive desktop and mobile navigation
- light-first theme behavior with manual dark mode
...
It is intentionally ungated and links back to `/brief/`.
...
Light is the intended first-load theme.
...
Known refinement: the static Brief HTML begins dark and deferred JavaScript applies the resolved light theme. This can produce a short dark flash.
...
- opens the soundtrack drawer when one direct tap is required

Automatic playback remains subject to browser and provider policy.

## Privacy and trust boundary

Current public routes are noindex demonstrations. Noindex is a crawler instruction, not access control.
...
Merged August 5 changes passed their relevant validation. The final direct-commit head did not return a combined status payload during this documentation audit, so release claims should be based on a fresh workflow run.
...
2. remove the possible dark first-paint flash
3. add a `/brief` and `/brief-next` parity guard
4. mock Spotify controller readiness and fallback states in tests
5. inventory legacy Brief assets before cleanup
6. define one release checklist
7. add documentation freshness checks
8. preserve clear demo and future-platform labels
