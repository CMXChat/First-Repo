...
COMPOSE=(
  ...
  --file "${BACKEND_DIR}/compose.secure.yml"
)
...
for attempt in $(seq 1 40); do
  if curl --fail --silent http://127.0.0.1:8000/api/health/live >/dev/null; then
    "${COMPOSE[@]}" ps
    echo "Staging containers are running with app.hardened:app."
    echo "Continue with Cloudflare Access denial, identity, browser, backup, and restore validation."
    exit 0
  fi
  sleep 2
done
