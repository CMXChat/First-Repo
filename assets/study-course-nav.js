(()=>{
  ...
  const sync=()=>{current.textContent=titleFor(chapters[index]);count.textContent=`${index+1} / ${chapters.length}`;bar.style.width=`${((index+1)/Math.max(1,chapters.length))*100}%`;save()};
  ...
  if(path==='/study/'){
    ...
    if(host&&!document.querySelector('.course-world-gate')){const gate=document.createElement('div');gate.className='course-world-gate';gate.innerHTML='<div><strong>The full course now has worlds</strong><p>Finish the core app-flow missions here, then deepen the backend in Python Lab and open the complete Environment World for every tool, folder, workflow, guardrail and later concept in the build setup.</p></div><div class="course-world-actions"><a href="/study/python/">Python Lab</a><a class="primary" href="/study/environment/">Full Environment World →</a></div>';host.insertAdjacentElement('afterend',gate)}
  }
})();