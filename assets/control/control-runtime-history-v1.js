(() => {
  ...
  const MAX_RUNS = 24;
  const state = { automations: [], runs: [], receipt: null, connected: false };
  ...
  const rows = (value) => Array.isArray(value)
    ...
    : Array.isArray(value?.items) ? value.items
      : Array.isArray(value?.runs) ? value.runs
        : Array.isArray(value?.automations) ? value.automations : [];
  ...
  const tone = (status) => ({
    succeeded: "good", failed: "bad", cancelled: "quiet", running: "active",
  })[status] || "waiting";
  ...
  function ensureSurface() {
    ...
    const historyView = document.querySelector('[data-cc-view="history"] .cc-full-view');
    ...
    section.innerHTML = `
      ...
      <div class="cc-runtime-grid">
        <section class="cc-runtime-list-panel" aria-labelledby="controlRuntimeRunsHeading">
          <div class="cc-runtime-panel-head"><div><small>Canonical Runtime</small><strong id="controlRuntimeRunsHeading">Recent runs</strong></div><span id="controlRuntimeCount">0</span></div>
          <div id="controlRuntimeRuns" class="cc-runtime-runs"><p class="cc-runtime-empty">Unlock to inspect durable Runtime history.</p></div>
        </section>
        <section class="cc-runtime-receipt" aria-labelledby="controlRuntimeReceiptHeading">
          <div class="cc-runtime-panel-head"><div><small>Frozen execution proof</small><strong id="controlRuntimeReceiptHeading">Receipt / Why</strong></div></div>
          <div id="controlRuntimeReceipt"><p class="cc-runtime-empty">Choose a Run to inspect its immutable execution references and Why timeline.</p></div>
        </section>
      </div>
    ...
    historyView.prepend(section);
    const sample = historyView.querySelector(".cc-list-card");
    if (sample) {
      sample.classList.add("cc-sample-history");
      const kicker = sample.querySelector(".cc-panel-kicker");
  ...
  function setBackend(kind, message) {
    ...
    if (el("controlRuntimeState")) el("controlRuntimeState").textContent = message;
  }
  ...
  function automationName(id) {
    const item = state.automations.find((automation) => String(automation.id) === String(id));
    return item?.name || item?.display_name || "Automation";
  }
  ...
  function renderRuns() {
    ...
    root.innerHTML = state.runs.map((run) => `
      <button class="cc-runtime-run" type="button" data-automation-id="${esc(run.automation_id)}" data-run-id="${esc(run.id)}">
        <span class="cc-runtime-run-top"><strong>${esc(automationName(run.automation_id))}</strong><span class="cc-runtime-status" data-tone="${tone(run.status)}">${esc(run.status || "unknown")}</span></span>
  ...
  function frozenSender(frozen) {
    const identity = frozen.sender_display_name || "Sender";
    const address = frozen.sender_address || "—";
    const connection = frozen.connection_display_name || "—";
    return `${identity} · ${address} · ${connection}`;
  }
  ...
  function renderReceipt(receipt) {
    ...
    const attempts = rows(receipt.attempts);
    const events = rows(receipt.events || receipt.why);
    ...
    root.innerHTML = `
      ...
      <div class="cc-runtime-section">
        ...
        <div class="cc-runtime-readable">
          ...
          <p><span>From</span><strong>${esc(frozenSender(frozen))}</strong></p>
      ...
      <div class="cc-runtime-section"><h4>Attempts</h4><div class="cc-runtime-timeline">
      ...
      <div class="cc-runtime-section"><h4>Why / execution timeline</h4><div class="cc-runtime-timeline">
  ...
  async function loadReceipt(automationId, runId) {
    ...
    try {
      ...
      const message = kind === "not_deployed_or_missing"
        ? "Receipt route is not deployed here, or this Run no longer resolves through the requested Automation."
        : `Receipt unavailable: ${error?.message || "request failed"}`;
      el("controlRuntimeReceipt").innerHTML = `<p class="cc-runtime-empty">${esc(message)}</p>`;
    }
  }
  ...
  async function loadHistory() {
    ...
    try {
      state.automations = rows(await api.listAutomations()).slice(0, MAX_AUTOMATIONS);
      const results = await Promise.allSettled(state.automations.map(async (automation) =>
        rows(await api.listRuns(automation.id)).map((run) => ({ ...run, automation_id: run.automation_id || automation.id }))));

      if (results.some((result) => result.status === "rejected" && api.classify(result.reason) === "locked")) {
        return showUnlock("Protected session expired while loading Runtime history. Unlock again.");
      }

      state.runs = results.filter((result) => result.status === "fulfilled")
        .flatMap((result) => result.value)
        .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0))
        .slice(0, MAX_RUNS);

      const unavailable = results.filter((result) => result.status === "rejected").length;
      showConnected(unavailable
        ? `Protected session ready · ${state.runs.length} recent Runs loaded · ${unavailable} Automation Run list${unavailable === 1 ? "" : "s"} unavailable.`
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (!button) return;
    if (button.id === "controlRuntimeRefresh") {
      button.disabled = true;
      try { await loadHistory(); } finally { button.disabled = false; }
    }
    if (button.id === "controlRuntimeLogout") {
      button.disabled = true;
      try {
        await api.logout();
        state.automations = [];
