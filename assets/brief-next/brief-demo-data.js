window.BRIEF_DEMO_DATA = {
  ...
  scenarios: {
    personal: {
      ...
      headline: 'Here is the shape of your day.',
    ...
    relationship: {
      ...
      details: {
        ...
        plans: { title: 'Shared decisions with owners and timing', summary: 'Plans stay useful when each decision has an owner and approval state.', cards: [{ label: 'Travel', title: 'Choose the hotel', detail: 'Jordan prepares options. Both approve.' }, { label: 'Budget', title: 'Confirm trip fund', detail: 'Needs both before any transfer.' }, { label: 'Ritual', title: 'Sunday planning call', detail: 'Recurring shared check-in.' }] },
    ...
    business: {
      ...
      recommendation: { label: 'Recommended next move', title: 'Collect before expanding fixed cost.', detail: 'Tie contractor approval to cash received, not optimistic pipeline value.' },
      ...
      details: {
        ...
        projects: { title: 'Risks, owners and deadlines', summary: 'Unowned work becomes visibly uncomfortable.', cards: [{ label: 'Watch', title: 'London capacity', detail: 'Amina owns the mitigation plan.' }, { label: 'At risk', title: 'Client approval', detail: 'Eli owns the follow-up.' }, { label: 'On track', title: 'Website release', detail: 'Two review notes remain.' }] },
    ...
    trainer: {
      ...
      weather: { location: 'Training window', temperature: 79, condition: 'Warm and dry', high: 81, low: 68, advice: 'Indoor training is comfortable. Hydrate before the session.', hourly: [{ time: 'Now', temp: 79, rain: 5 }, { time: '6 PM', temp: 78, rain: 6 }, { time: '8 PM', temp: 73, rain: 8 }, { time: 'Late', temp: 69, rain: 9 }] },
    ...
    team: {
      id: 'team', label: 'Team and project', short: 'Roles, handoffs and shared project truth', greeting: 'Good afternoon, Atlas team.', headline: 'Everyone should see the same mission without seeing everything.', summary: 'Members receive role-relevant work, while project leads keep the broader operating picture and restricted context.',
      ...
      details: {
        mywork: { title: 'Role-relevant work without the whole company', summary: 'A member sees what they own, what blocks it and the approved context they need.', cards: [{ label: 'Next', title: 'Finish responsive review', detail: 'Owner: Sam. Due before readiness review.' }, { label: 'Dependency', title: 'Content approval', detail: 'Waiting on Jordan.' }, { label: 'Context', title: 'Release goal', detail: 'Protect the mobile experience.' }] },
        project: { title: 'One shared timeline with visible blockers', summary: 'The status changes when evidence changes, not when the presentation wants to look optimistic.', cards: [{ label: 'Complete', title: 'Core build', detail: 'Accepted by the project lead.' }, { label: 'Watch', title: 'Mobile review', detail: 'Two issues remain.' }, { label: 'Blocked', title: 'Final handoff', detail: 'Receiver not confirmed.' }] },
