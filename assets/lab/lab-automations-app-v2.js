(() => {
  ...
  function blankRepeat(){ return {mode:"none",every:1,unit:"days"}; }
  ...
  function legacyRepeat(repeat){
    if (repeat === "daily") return {mode:"daily",every:1,unit:"days"};
    if (repeat === "until_ack") return {mode:"until_ack",every:1,unit:"days"};
    return blankRepeat();
  }
  ...
  function repeatLabel(item){const r=item.repeatConfig;if(!r||r.mode==="none")return"Do not repeat";if(r.mode==="daily")return"Repeat daily";if(r.mode==="weekly")return"Repeat weekly";if(r.mode==="until_ack")return"Repeat until acknowledged";return`Every ${r.every||1} ${r.unit||"days"}`;}
  ...
  function timezoneDropdown(item){return dropdownMarkup({id:"timezone",label:"Timezone",value:item.timing.at.timezone||localZone,placeholder:localZone,options:TIMEZONES.map(z=>({value:z,label:z===localZone?`${z} (device)`:z,meta:z==="UTC"?"Coordinated Universal Time":"IANA timezone"}))});}
