(() => {
  ...
  const CALENDAR_REPEAT_UNITS = new Set(["days","weeks","months","years"]);
  ...
  function legacyRepeat(repeat){
    ...
    if (repeat === "weekly") return {mode:"weekly",every:1,unit:"weeks",timezone:localZone};
  ...
  function isCalendarRepeat(r){return r?.mode==="daily"||r?.mode==="weekly"||(r?.mode==="custom"&&CALENDAR_REPEAT_UNITS.has(r.unit));}
  function repeatLabel(item){
    const r=item.repeatConfig;
    if(!r||r.mode==="none")return"Do not repeat";
    if(r.mode==="daily")return`Repeat daily · ${r.timezone||localZone}`;
    if(r.mode==="weekly")return`Repeat weekly · ${r.timezone||localZone}`;
    if(r.mode==="until_ack")return"Repeat until acknowledged";
    const zone=isCalendarRepeat(r)?` · ${r.timezone||localZone}`:"";
    return`Every ${r.every||1} ${r.unit||"days"}${zone}`;
  }
  ...
  function timezoneOptions(){return TIMEZONES.map(z=>({value:z,label:z===localZone?`${z} (device)`:z,meta:z==="UTC"?"Coordinated Universal Time":"IANA timezone"}));}
  function timezoneDropdown(item){return dropdownMarkup({id:"timezone",label:"Timezone",value:item.timing.at.timezone||localZone,placeholder:localZone,options:timezoneOptions()});}
  function repeatTimezoneDropdown(item){return dropdownMarkup({id:"repeat-timezone",label:"Calendar timezone",value:item.repeatConfig.timezone||localZone,placeholder:localZone,options:timezoneOptions()});}
  function repeatControls(item){
    const r=item.repeatConfig;
    const modes=[["none","Do not repeat"],["daily","Daily"],["weekly","Weekly"],["custom","Custom cadence"],["until_ack","Until acknowledged"]];
    const calendarCadence=isCalendarRepeat(r);
    const customCalendar=r.mode==="custom"&&CALENDAR_REPEAT_UNITS.has(r.unit);
    const customNote=customCalendar
      ? "Days, weeks, months and years follow calendar boundaries in the selected timezone. Minutes and hours are elapsed cadence units."
      : "Minutes and hours are elapsed cadence units. Choose days, weeks, months or years for calendar-aware recurrence.";
    const presetNote=r.mode==="daily"
      ? "Daily recurrence keeps its local wall-clock anchor in the selected timezone."
      : "Weekly recurrence keeps its local weekday/time anchor in the selected timezone.";
    return `<div class="repeat-block"><div class="timing-title"><div><h3>Repeat</h3><small>Optional cadence after the action becomes due.</small></div></div><div class="repeat-pills">${modes.map(([m,l])=>`<button type="button" class="repeat-pill ${r.mode===m?"is-selected":""}" data-repeat-mode="${m}">${l}</button>`).join("")}</div>${r.mode==="custom"?`<div class="custom-repeat"><label class="time-number"><span>Every</span><input type="number" min="1" max="999" inputmode="numeric" value="${Number(r.every)||1}" data-repeat-every></label>${dropdownMarkup({id:"repeat-unit",label:"Unit",value:r.unit||"days",placeholder:"days",options:REPEAT_UNITS.map(x=>({value:x,label:x}))})}</div>`:""}${calendarCadence?`<div class="repeat-zone">${repeatTimezoneDropdown(item)}</div>`:""}${r.mode==="custom"?`<div class="calendar-cadence-note"><strong>${customCalendar?"Calendar cadence":"Elapsed cadence"}</strong><span>${customNote}</span></div>`:""}${r.mode==="daily"||r.mode==="weekly"?`<div class="calendar-cadence-note"><strong>Calendar cadence</strong><span>${presetNote}</span></div>`:""}</div>`;
  }
  function renderTiming(item){
    const t=item.timing;const exactReady=t.at.date&&t.at.time;
    return `<section class="step-section"><header class="step-heading"><small>WAIT / REPEAT</small><h2>When should the next step happen?</h2><p>Use a shortcut, enter an exact delay, or wait until a specific date and minute.</p></header><div class="timing-mode-grid">${timingModeButton("none","No wait","→",item)}${timingModeButton("delay","Delay","⌛",item)}${timingModeButton("exact","Exact date & time","◷",item)}</div>${t.mode==="delay"?`<article class="precision-card"><div class="timing-title"><div><h3>Delay</h3><small>Set the elapsed duration precisely.</small></div><span>${esc(timingLabel(item))}</span></div><div class="preset-row"><button type="button" data-delay-preset="15m">15 min</button><button type="button" data-delay-preset="1h">1 hour</button><button type="button" data-delay-preset="6h">6 hours</button><button type="button" data-delay-preset="24h">24 hours</button></div><div class="duration-inputs">${numberField("Days","days",t.delay.days,365)}${numberField("Hours","hours",t.delay.hours,23)}${numberField("Minutes","minutes",t.delay.minutes,59)}</div></article>`:""}${t.mode==="exact"?`<article class="precision-card"><div class="timing-title"><div><h3>Exact date & time</h3><small>Minute precision. The backend will later store an authoritative timestamp plus timezone.</small></div><span>${exactReady?"READY":"SET DATE + TIME"}</span></div><div class="exact-grid"><label class="field"><span>Date</span><input type="date" value="${esc(t.at.date)}" data-exact-date></label><label class="field"><span>Time</span><input type="time" step="60" value="${esc(t.at.time)}" data-exact-time></label>${timezoneDropdown(item)}</div><div class="time-preview"><small>WAIT UNTIL</small><strong>${exactReady?esc(`${t.at.date} at ${t.at.time} · ${t.at.timezone}`):"Choose a date and time"}</strong></div></article>`:""}${repeatControls(item)}<div class="backend-note"><strong>Future backend:</strong> WAIT delays are elapsed durations, so a 2-day wait means 48 elapsed hours. Recurrence is separate: minutes/hours are elapsed cadence, while days/weeks/months/years preserve calendar wall-clock intent in an IANA timezone. Calendar recurrence needs explicit DST rules, and monthly/yearly recurrence also needs end-of-month/leap-day rules. The browser never sleeps until the time arrives.</div></section>`;
  }
  ...
  function bindCommon(){
    ...
    document.querySelectorAll("[data-repeat-mode]").forEach(b=>b.addEventListener("click",()=>{state.editing.repeatConfig.mode=b.dataset.repeatMode;if(b.dataset.repeatMode==="daily")state.editing.repeatConfig.unit="days";if(b.dataset.repeatMode==="weekly")state.editing.repeatConfig.unit="weeks";scheduleAutosave();renderEditor();}));
