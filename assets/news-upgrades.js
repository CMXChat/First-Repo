(() => {
  ...
  function makeSection({ id, path, title, badge, badgeAudience = 'shared', intro = '', rootClass = 'card-grid', keepVisible = false }) {
  ...
  function staticCard({ label, title, text, audience = 'shared', className = '', items = [], details = null }) {
    const card = createCard({
      ...
      audience,
      items,
      cardClass: `permanent-card ${className}`.trim()
    });
  ...
  function renderPermanentSections() {
    const whyRoot = document.getElementById('whyExistsCards');
    whyRoot?.appendChild(staticCard({
      label: 'the purpose',
    ...
    if (visionRoot) {
      const card = staticCard({
        ...
        text: 'This briefing is also a working prototype. The goal is to bring together the information, music, priorities, memories, and updates that matter to each person without making them search through five different apps.',
    ...
    if (comingRoot) {
      const card = staticCard({
        ...
        text: 'Once a secure backend is connected, the page can become interactive. Crystal will be able to suggest ideas, react to sections, vote on songs, save memories, influence future briefings, and request corrections without pretending any of those controls work today.'
      });
      const features = document.createElement('div');
      features.className = 'future-feature-list';
      ['Reactions', 'Song voting', 'Notes', 'Suggestions', 'Saved memories', 'Search', 'Shared checklists', 'Daily answers', 'Feedback', 'Private settings'].forEach(value => {
        const chip = document.createElement('span');
        chip.textContent = value;
      ...
      note.textContent = 'These are product directions, not active controls. The current page remains a private static prototype until server-side authentication and storage exist.';
  ...
  function applyOrder() {
    const fallbackOrder = [
      'sinceYesterday', 'priority', 'spotify', 'opening', 'activity', 'whatBuilt', 'behindBuild',
      'crystal', 'weather', 'style', 'culture', 'local', 'world', 'relationship', 'timeline',
      'memory', 'whyExists', 'productVision', 'progress', 'nextSteps', 'comingSoon', 'horoscope',
      'quote', 'questions', 'livingPrototype'
    ];
    const order = Array.isArray(brief.meta?.sectionOrder) ? brief.meta.sectionOrder : fallbackOrder;
    const sections = new Map([...host.querySelectorAll('.brief-section')]
  ...
  [
    ...
    makeSection({ id: 'whatBuilt', path: '~/jay/what-changed', title: 'WHAT JAY BUILT', badge: 'plain language', badgeAudience: 'jay', rootClass: 'card-grid' }),
    ...
    makeSection({ id: 'memory', path: '~/shared/memory', title: 'MEMORY OF THE DAY', badge: 'one moment', rootClass: 'card-grid' }),
    makeSection({ id: 'whyExists', path: '~/brief/purpose', title: 'WHY THIS EXISTS', badge: 'permanent', rootClass: 'card-grid', keepVisible: true }),
    makeSection({ id: 'productVision', path: '~/product/vision', title: 'WHAT THIS COULD BECOME', badge: 'working prototype', rootClass: 'card-grid', keepVisible: true }),
    makeSection({ id: 'progress', path: '~/shared/progress', title: 'PROGRESS TRACKER', badge: 'four areas', rootClass: 'progress-grid' }),
    makeSection({ id: 'nextSteps', path: '~/brief/next', title: 'WHAT’S NEXT', badge: 'keep it practical', rootClass: 'card-grid' }),
    makeSection({ id: 'comingSoon', path: '~/product/coming-soon', title: 'COMING SOON', badge: 'backend later', rootClass: 'card-grid', keepVisible: true }),
    makeSection({ id: 'livingPrototype', path: '~/product/live-prototype', title: 'LIVING PROTOTYPE', badge: 'building in public, privately', rootClass: 'card-grid', keepVisible: true })
  ...
  renderCards('memory', brief.memory, 'brief-card');
