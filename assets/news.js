(() => {
  ...
  function renderTimes() {
    ...
    const update = () => {
      ...
      zones.forEach(([timeId, dateId, timeZone]) => {
        setText(timeId, new Intl.DateTimeFormat('en-US', { timeZone, hour: 'numeric', minute: '2-digit' }).format(now));
        setText(dateId, new Intl.DateTimeFormat('en-US', { timeZone, weekday: 'short', month: 'short', day: 'numeric' }).format(now));
      });
    };
    update();
  ...
  function render() {
    ...
    renderCards('styleCards', brief.style, 'brief-card');
    renderCards('spotifyCards', brief.spotify, 'brief-card');
