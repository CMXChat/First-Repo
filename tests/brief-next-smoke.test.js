...
assert.equal((html.match(/<script src=/g) || []).length, 6, 'The briefing should load data, Spaces branding, experience, media, app and explainer scripts.');
assert.equal((html.match(/<link rel="stylesheet"/g) || []).length, 5, 'The briefing should load core, explainer, experience, Doc-link and topbar stylesheets.');
...
assert.equal(stagingRoute.name, 'Spaces Brief Staging Copy');
...
console.log('Spaces Brief static smoke test passed.');
