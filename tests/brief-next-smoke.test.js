...
const docHtml = read('doc/index.html');
...
assert.match(html, /id="openDemoLabel">Choose a Briefing/);
...
assert.match(html, /id="priorityRoutingButton"/);
assert.match(html, /id="priorityRoutingDialog"/);
assert.match(html, /Choose where priority alerts should go/);
assert.match(html, /<strong>Preview only\.<\/strong> No message leaves this demo/);
...
assert.match(experienceJs, /class="everything-progress-rail" role="progressbar"/);
assert.match(experienceJs, /function scrollToEverythingSection\(/);
assert.match(experienceJs, /aria-current', 'location'/);
assert.match(experienceJs, /class="full-signal-console"/);
assert.match(experienceJs, /class="full-space-map"/);
assert.match(experienceJs, /class="morning-concept-stage"/);
assert.match(experienceJs, /class="privacy-control-console"/);
assert.match(experienceJs, /class="full-doc-bridge"/);
assert.match(experienceJs, /href="\/doc\/"/);
assert.doesNotMatch(experienceJs, /style="--/);
assert.doesNotMatch(advancedJs, /style="--/);
...
assert.deepEqual(Object.keys(data.alertRoutes), ['personal', 'relationship', 'family', 'business', 'accounting', 'trainer', 'team']);
for (const scenario of Object.values(data.scenarios)) {
  assert.ok(scenario.entryPreview?.title, `${scenario.id} should explain its entrance preview.`);
  assert.equal(scenario.entryPreview?.metrics.length, 3, `${scenario.id} should include three entrance signals.`);
  assert.ok(scenario.entryPreview?.privateLabel, `${scenario.id} should name its private boundary.`);
  assert.ok(scenario.entryPreview?.sharedLabel, `${scenario.id} should name its approved shared scope.`);
  ...
  assert.ok(route?.rule, `${scenario.id} should explain its alert rule.`);
  assert.equal(route?.channels.length, 4, `${scenario.id} should preview four governed delivery options.`);
  assert.ok(route?.channels.some(channel => channel.active), `${scenario.id} should start with at least one example route.`);
}
assert.equal(data.alertRoutes.family.channels[0].destination, 'Home Base');
assert.equal(data.alertRoutes.team.channels[0].destination, 'Launch room');
...
assert.match(appJs, /function renderPriorityRoutingDialog\(/);
assert.match(appJs, /function togglePriorityChannel\(/);
assert.match(appJs, /Demo routing saved\. No message was sent\./);
...
assert.match(exploreJs, /The selected category stays open while the other/);
assert.match(exploreJs, /Open another category when you need it/);
...
assert.match(exploreCss, /\.workspace-preview-grid/);
assert.match(exploreCss, /\.workspace-preview-card/);
assert.match(exploreCss, /\.briefing-settings-dialog/);
assert.match(exploreCss, /\.today-focus-nav/);
...
assert.match(docHtml, /A Space stays current through connected accounts, direct input, and corrections/);
assert.match(docHtml, /Planned Memory &amp; Data settings/);
assert.match(docHtml, /Cloudflare’s Agents SDK could become infrastructure beneath Spaces/);
...
assert.match(docLinksCss, /brief-demo-topbar-polish\.css\?v=20260808-1/);
...
assert.match(topbarPolishCss, /\.spaces-theme-toggle \.theme-toggle-track/);
assert.match(topbarPolishCss, /\.spaces-theme-toggle \.theme-toggle-thumb/);
