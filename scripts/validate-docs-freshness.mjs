...
if (baseSha && !/^0+$/.test(baseSha)) {
  ...
  const docsChanged = changed.some(file => currentDocs.includes(file) || file === 'docs/personal-os-release-safeguards.md');

  if (materialChange && !docsChanged) {
    fail('Material Personal OS product files changed without updating a current operational document.');
  }
}
...
console.log(`Documentation freshness validation passed for ${currentDocs.length} operational documents.`);
