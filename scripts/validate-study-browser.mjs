...
async function waitJson(url,tries=40){for(let i=0;i<tries;i++){try{const r=await fetch(url);if(r.ok)return r.json()}catch{}await sleep(200)}throw new Error(`Timed out waiting for ${url}`)}
await waitJson('http://127.0.0.1:8787/study/');
...
for(const [vp,width,height] of viewports){
  ...
  for(const route of routes){
    ...
    await cdp('Page.navigate',{url});await sleep(1400);
